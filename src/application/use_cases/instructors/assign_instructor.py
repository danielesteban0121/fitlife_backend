...
from src.application.dtos.instructor_dtos import AssignInstructorRequestDTO, AssignInstructorResponseDTO
import uuid
...
class AssignInstructor:
    ...
    async def execute(self, request: AssignInstructorRequestDTO) -> AssignInstructorResponseDTO:
        ...
        active_assignment = await self.instructor_repository.get_active_assignment_for_user(request.user_id)
        if active_assignment:
            ...
            prev_instructor = await self.instructor_repository.find_by_id(active_assignment.instructor_id)
