...
def get_create_routine(db: AsyncSession = Depends(get_db)) -> CreateRoutine:
    return CreateRoutine(SQLAlchemyTrainingRepository(db), SQLAlchemyUserRepository(db), SQLAlchemyInstructorRepository(db))
...
def get_create_nutrition_plan(
    ...
    return CreateNutritionPlan(SQLAlchemyNutritionRepository(db), SQLAlchemyUserRepository(db), SQLAlchemyInstructorRepository(db), notif)
